...
const { buildPdfBuffer, buildPdfBufferFromStructuredCv, buildPdfBufferLenient, normalizeToCvTemplateText, __test } = require('../netlify/functions/pdf-builder');
...
async function run() {
  ...
  const inlineHeadingTextCv = [
    'Mona Lee',
    'Program Director',
    'Riyadh, Saudi Arabia | +966 50 222 2222 | mona@example.com',
    '',
    'Executive Summary: Transformational HR and learning leader with regional delivery experience.',
    '',
    'Key Skills: Leadership Development | Change Enablement | Stakeholder Management',
  ].join('\n');
  const canonicalInlineHeading = normalizeToCvTemplateText(inlineHeadingTextCv);
  assert.ok(!/Executive Summary:/i.test(canonicalInlineHeading), 'Normalized output should strip inline heading labels from body text.');
  assert.ok(!/Key Skills:/i.test(canonicalInlineHeading), 'Inline skills heading label should not be duplicated in body content.');

  const orphanPrefaceCv = [
    'Confidential Draft - Internal Review Copy',
    'Generated by System Export Tool',
    'Nadia Khan',
    'Head of Talent Development',
    'Doha, Qatar | +974 55 111 1111 | nadia@example.com',
    '',
    'Professional Summary',
    'Strategic talent leader with cross-border experience.',
  ].join('\n');
  const orphanStructured = __test.validateAndAutoCorrect(__test.buildStructuredCvObject(orphanPrefaceCv));
  assert.strictEqual(orphanStructured.fullName, 'Nadia Khan', 'Orphan preface fragments should be suppressed from identity fields.');
  assert.strictEqual(orphanStructured.professionalTitle, 'Head of Talent Development', 'Title extraction should remain clean with orphan preface text.');

  const unsafeIdentityCv = [
    'Strategic leader with 18+ years transforming enterprise capability and stakeholder outcomes across markets.',
    'Additional profile sentence with broad skills, expertise, and achievements.',
    'SELECTED ACHIEVEMENTS',
    '- Improved leadership bench strength by 40%.',
  ].join('\n');
  const lenientPdf = buildPdfBufferLenient(unsafeIdentityCv);
  const lenientContent = decodePdf(lenientPdf);
  assert.ok(!lenientContent.includes('/F2 16 Tf\n(Strategic leader with 18+ years'), 'Lenient fallback must not render polluted summary prose as name.');
  assert.ok(lenientContent.includes('(CV Candidate) Tj'), 'Lenient fallback should fail safe with a clean placeholder identity.');
